            ...
            <div id="Material" class="tab-pane fade in active">
              ...
              <div class="table-responsive table-wrapper-scroll-y" style="height:280px;margin-top:10px;" >
                <table id="example2" class="display nowrap table table-striped table-bordered itemlist" style="height:auto !important;">
                  ...
                  <tbody>
                    @if(!empty($getFranch))
                      @foreach($getFranch as $key=>$row)

                    <tr class="participantRow">
                      <td><input {{$ActionStatus}} type="checkbox" name="FRANCHISE_REF[]" value="{{$row->BRID}}" class="checkbox" {{ in_array($row->BRID, $FranchList)? 'checked':'' }}></td>
                      <td><input {{$ActionStatus}} type="text"     name="FRANCHISE_CODE[]" id="FRANCHISE_CODE_{{$key}}" value="{{$row->BRCODE}}" class="form-control showEmp" readonly  style="width:100%;"  /></td>
                      <td><input {{$ActionStatus}} type="text"     id ="FRANCHISE_NAME_{{$key}}" value="{{$row->BRNAME}}" class="form-control w-100" maxlength="200" readonly style="width:100%;" ></td>
                    </tr>
