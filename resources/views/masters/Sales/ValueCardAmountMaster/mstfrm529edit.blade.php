    ...
    <div class="container-fluid filter">
      <div class="inner-form">         
        <div class="row">
          ...
          <div class="col-lg-2 pl">
            <input {{$ActionStatus}} type="text" name="VALUE_CARD_AMOUNT" id="VALUE_CARD_AMOUNT" onkeyup="onChangeAmount(this.id,this.value)" value="{{isset($HDR->VCA_AMT)?$HDR->VCA_AMT:''}}" class="form-control" autocomplete="off" onkeypress="return isNumberDecimalKey(event,this)" >                                                       
          </div>   
        </div>
        ...
        <div class="row">
          ...
          <div class="col-lg-2 pl">
            <input {{$ActionStatus}} type="text" name="DIS_PERCENT" id="DIS_PERCENT" onkeyup="onChangeAmount(this.id,this.value)" value="{{isset($HDR->DIS_PERCENT)?$HDR->DIS_PERCENT:''}}"  class="form-control" autocomplete="off" onkeypress="return isNumberDecimalKey(event,this)" >                                                       
          </div>
        ...
        <div class="row">
          ...
          <div class="col-lg-2 pl">
            <select {{$ActionStatus}} name="CARD_VALIDITY_MODE" id="CARD_VALIDITY_MODE" class="form-control" autocomplete="off" >
              <option value="">Select</option>
              <option {{isset($HDR->CARD_VALIDITY_MODE) && $HDR->CARD_VALIDITY_MODE =='DAY'?'selected="selected"':''}} value="DAY">DAY</option>
              <option {{isset($HDR->CARD_VALIDITY_MODE) && $HDR->CARD_VALIDITY_MODE =='MONTH'?'selected="selected"':''}} value="MONTH">MONTH</option>
            </select>
          </div>

          <div class="col-lg-2 pl"><p>Card Validity in day/month*</p></div>
          <div class="col-lg-2 pl">
            <input {{$ActionStatus}} name="CARD_VALIDITY" id="CARD_VALIDITY" value="{{isset($HDR->CARDVALIDITY_MON)?$HDR->CARDVALIDITY_MON:''}}" class="form-control" autocomplete="off" onkeypress="return isNumberKey(event,this)" >
          </div>
...
function onChangeAmount(textid,textval){
  ...
  if($.trim(textval) !="" && parseFloat($.trim(textval)) >= 0 ){
    ...
    var val_card_amt			  =	  $("#VALUE_CARD_AMOUNT").val();
    var disc_percent			  =	  $("#DIS_PERCENT").val();

    var dis_per_amt			    =	  $("#DIS_PER_AMT").val();
    var tot_amt		          =	  $("#TOTAL_AMOUNT").val();

    var tot_disc_percent        =   parseFloat((parseFloat(val_card_amt)*parseFloat(disc_percent))/100).toFixed(2);
    var all_tot_amt     		    = 	parseFloat(parseFloat(val_card_amt) - parseFloat(disc_percent));

    
   
    $("#DIS_PER_AMT").val(tot_disc_percent);
    $("#TOTAL_AMOUNT").val(all_tot_amt);
    //$("#DIS_PER_AMT").val(all_tot_amt);
