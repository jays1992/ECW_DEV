      ...
      if(data.length > 0){
        $.each(data, function(key, value) {
         ...
         var check_dactive  = value.ACTIVE_DEACTIVE =='0'?'selected="selected"':'';
          ...
          html +='<td style="text-align:center;"> <a href="{{route("master",[$FormId,"searchcard"]) }}/'+window.btoa(value.DETAIL_ID+'_'+value.BRANCH_ID)+'" class="btn checkstore"  id="0" target="_blank"><i class="fa fa-clone"></i></a> </td>';
          html +='</tr>';
        });
      }
      else{
