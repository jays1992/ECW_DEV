...
<div class="container-fluid topnav">
  <div class="row">
    ...
    <div class="col-lg-10 topnav-pd">
      ...
      <button class="btn topnavbt" id="btnSaveFormData" onclick="saveAction('save')" ><i class="fa fa-floppy-o"></i> Send</button>
  ...
  <div class="container-fluid filter"> 
    ...
    <div class="inner-form"> 
      ...
      <div class="row">                  
        ...
        <div class="col-lg-2 pl">
          <select name="CAMPAIGN_TYPE" id="CAMPAIGN_TYPE" class="form-control" autocomplete="off" onchange="resetTemplate()" >
        ...
        <div class="col-lg-2 pl">
          <select name="DATA_SELECTION" id="DATA_SELECTION" class="form-control" autocomplete="off" onchange="resetNoOfDays()" >
        ...
        <div class="col-lg-2 pl">
          <input type="text" name="NO_OF_DAYS" id="NO_OF_DAYS" class="form-control" autocomplete="off" onkeypress="return isNumberKey(event,this)" readonly >
        </div>
      </div>
      ...
      <div class="row">                  
        ...
        <div class="col-lg-2 pl">
          <input type="text"   name="TEMPLATE_NAME" id="TEMPLATE_NAME"  class="form-control"  autocomplete="off"  onclick="getTemplateMaster()" readonly/>
          <input type="hidden" name="TEMPLATE_ID"   id="TEMPLATE_ID"    class="form-control"  autocomplete="off" />  
        </div>
      </div>
...
function getTemplateMaster(){
  ...
  $.ajax({
    url:'{{route("master",[$FormId,"getTemplateMaster"])}}',
    type:'POST',
    data:{'CAMPAIGN_TYPE':$("#CAMPAIGN_TYPE").val()},
    success:function(data) {
      var html = '';
      ...
      if(data.length > 0){
        $.each(data, function(key, value) {
          html +='<tr>';
          html +='<td style="width:10%;text-align:center;" ><input type="checkbox" id="key_'+key+'" value="'+value.DATA_ID+'" onChange="bindTemplateMaster(this)" data-code="'+value.DATA_CODE+'" data-desc="'+value.DATA_DESC+'" ></td>';
          html +='<td style="width:45%;" >'+value.DATA_CODE+'</td>';
          html +='<td style="width:45%;" >'+value.DATA_DESC+'</td>';
          html +='</tr>';
        });
  ...
  $("#modal_title").text('Message Template');
  $("#modal_th1").text('Code');
  $("#modal_th2").text('Descripation');
  $("#modal").show();
}

function bindTemplateMaster(data){
  
  var code  = $("#"+data.id).data("code");
  var desc  = $("#"+data.id).data("desc");

  $("#TEMPLATE_ID").val(data.value);
  $("#TEMPLATE_NAME").val(code+' - '+desc);
...
function validateForm(action){
  ...
  if($.trim($("#DOC_NO").val()) ===""){
    ...
    $("#NoBtn").hide();
    $("#OkBtn1").show();
    $("#AlertMessage").text('Please select franchise name.');
    $("#alert").modal('show');
    $("#OkBtn1").focus();
    return false;
  }
  else if($.trim($("#DATA_SELECTION").val()) ==="NOT VISITED" && $.trim($("#NO_OF_DAYS").val()) ===""){
    $("#FocusId").val('NO_OF_DAYS');        
    $("#YesBtn").hide();
    $("#NoBtn").hide();
    $("#OkBtn1").show();
    $("#AlertMessage").text('Please enter no of days.');
    $("#alert").modal('show');
    $("#OkBtn1").focus();
    return false;
  }  
  else if($.trim($("#TEMPLATE_ID").val()) ===""){
    $("#FocusId").val('TEMPLATE_NAME');        
    $("#YesBtn").hide();
    $("#NoBtn").hide();
    $("#OkBtn1").show();
    $("#AlertMessage").text('Please select template name.');
    $("#alert").modal('show');
    $("#OkBtn1").focus();
    return false;
  }
...
function resetNoOfDays(){
  $("#NO_OF_DAYS").val('');
  $("#NO_OF_DAYS").prop('readonly',true);
  if($("#DATA_SELECTION").val() ==='NOT VISITED'){
