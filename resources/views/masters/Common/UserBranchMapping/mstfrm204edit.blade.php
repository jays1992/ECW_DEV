            ...
            <table id="example2" class="display nowrap table table-striped table-bordered itemlist" width="100%" style="height:auto !important;">
              ...
              <tbody>
                ...
                <tr class="participantRow">
                    <td><input type="checkbox" name="DATA_ID[{{$key}}]" onClick="GetUserDetails(this.id,this.value,{{$row->DATA_ID ==$row->DATA_ID_REF?'true':'false'}})" id="RECORDID_{{$key}}" {{$row->DATA_ID ==$row->DATA_ID_REF?'checked':''}} value="{{$row->DATA_ID}}" class="checkbox" ></td>
...
function GetUserDetails(id,value,status){
  ...
  url:'{{route("master",[$FormId,"GetUserDetails"])}}',
  async: false,
  dataType: 'json',
