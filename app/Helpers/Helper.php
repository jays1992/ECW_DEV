...
{ 
	...
	public static function get_gldetail_PaymentReceipt($ID,$Module){
        if($Module=='Receipt'){
          $objRCPTACCOUNT = DB::select("SELECT  ISNULL(B.GLNAME, C.SLNAME) AS GLNAME FROM TBL_TRN_RECEIPT_ACCOUNT A LEFT JOIN TBL_MST_GENERALLEDGER B ON A.GLID_REF = B.GLID 
		  AND A.SLGL_TYPE='G'  LEFT JOIN TBL_MST_SUBLEDGER C ON A.GLID_REF = C.SGLID AND A.SLGL_TYPE='S' WHERE A.RECEIPTID_REF=? ",[$ID]);
		  
			
			//	dd($objRCPTACCOUNT);
        if($objRCPTACCOUNT){
                return $GL=$objRCPTACCOUNT[0]->GLNAME;
                }else{
                return Null;
                }
        }else if($Module=='Payment'){
            $objRCPTACCOUNT = DB::select("SELECT  ISNULL(B.GLNAME, C.SLNAME) AS GLNAME FROM TBL_TRN_PAYMENT_ACCOUNT A LEFT JOIN TBL_MST_GENERALLEDGER B ON A.GLID_REF = B.GLID 
		  AND A.SLGL_TYPE='G'  LEFT JOIN TBL_MST_SUBLEDGER C ON A.GLID_REF = C.SGLID AND A.SLGL_TYPE='S' WHERE A.PAYMENTID_REF=? ",[$ID]); 
            if($objRCPTACCOUNT){  
                 return $GL=$objRCPTACCOUNT[0]->GLNAME;
                }else{
                return Null;
            ...
            $objRCPTACCOUNT = DB::select("SELECT  ISNULL(B.GLNAME, C.SLNAME) AS GLNAME FROM TBL_TRN_FJRV01_ACC A LEFT JOIN TBL_MST_GENERALLEDGER B ON A.GLID_REF = B.GLID 
		  AND A.SGLID_REF='G'  LEFT JOIN TBL_MST_SUBLEDGER C ON A.GLID_REF = C.SGLID AND A.SGLID_REF='S' WHERE A.JVID_REF=? ",[$ID]);
			...
			$objRCPTACCOUNT = DB::select("SELECT  ISNULL(B.GLNAME, C.SLNAME) AS GLNAME FROM TBL_TRN_MJRV01_ACC A LEFT JOIN TBL_MST_GENERALLEDGER B ON A.GLID_REF = B.GLID 
			AND A.SGLID_REF='G'  LEFT JOIN TBL_MST_SUBLEDGER C ON A.GLID_REF = C.SGLID AND A.SGLID_REF='S' WHERE A.MJVID_REF=? ",[$ID]);
