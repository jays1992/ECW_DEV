...
class TrnFrm63Controller extends Controller
{
    ...
    public function index(){    
        ...
        $objDataList	=	DB::select("select hdr.POID,hdr.PO_NO,hdr.PO_DT,hdr.PO_VRF,hdr.PO_VTO,hdr.INDATE,CONCAT(T3.BRCODE,' - ',T3.BRNAME) AS BRANCH_NAME,
                            (
                            ...
                            LEFT JOIN TBL_MST_BRANCH T3 ON T3.BRID=hdr.BRID_REF 
    ...
    public function add(){    
        ...
        ->where('BRID','=',$BRID_REF)
        ->where('STATUS','=',$Status)
        ->select('TBL_MST_BRANCH.BRID','TBL_MST_BRANCH.BRCODE','TBL_MST_BRANCH.BRNAME')
        ->first();
        ...
        'objCOMPANY','objlastdt','objTemplateMaster','TabSetting','doc_req','docarray','Franchise','objBRANCH']));      
   ...
   public function save(Request $request) {
    ...
    public function edit($id=NULL){
        ...
        if(!is_null($id)){
            $objPO = DB::table('TBL_TRN_PROR01_HDR')
                    ...
                    ->leftJoin('TBL_MST_BRANCH', 'TBL_MST_BRANCH.BRID', '=', 'TBL_TRN_PROR01_HDR.BRID_REF')
                    ->select('TBL_TRN_PROR01_HDR.*','TBL_MST_BRANCH.BRCODE','TBL_MST_BRANCH.BRNAME')
                    ->first();
        ...
        return view('transactions.Purchase.PurchaseOrder.trnfrm63edit',compact(['AlpsStatus','objPO','objRights','objCount1',
           ...
           'objPBTDS','objCount6','objTemplateMaster','Template','objlastdt','TabSetting','FormId','Franchise'
        ]));
        }
    ...
    public function view($id=NULL){
        ...
        return view('transactions.Purchase.PurchaseOrder.trnfrm63view',compact(['AlpsStatus','objPO','objRights','objCount1',
           ...
           'objShpAddress','objBillAddress','objItems','objUOM','objItemUOMConv','objTNCDetails','objUdfPOData2',
           'objCalHeader','objCalDetails','TAXSTATE','POBASEDON','objCOMPANY','objCountAttachment','ActionStatus',
           'objPBTDS','objCount6','objTemplateMaster','Template','objlastdt','TabSetting','Franchise'
        ]));
        }
   ...
   try {
    ...
    public function amendment($id=NULL){
        ...
        return view('transactions.Purchase.PurchaseOrder.trnfrm63amendment',compact(['AlpsStatus','objPO','objRights','objCount1',
           ...
           'ActionStatus','TabSetting','Franchise'
        ]));
        }
    ...
    public function getFranchise(){
        ...
        $Franchise = DB::select("SELECT BRID,BRCODE,BRNAME FROM TBL_MST_BRANCH WHERE CYID_REF=$CYID_REF AND ( DEACTIVATED IS NULL OR DEACTIVATED = 0 )");
