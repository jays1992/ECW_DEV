...
class MstFrm147Controller extends Controller
{
    ...
    protected   $messages = [
                    ...
                    'PRICE_LEVEL_REF.required' => 'Required field',
                ];
    ...
    public function index(){
        ...
        $objDataList=DB::select("SELECT * FROM TBL_MST_BRANCH WHERE CYID_REF='$CYID_REF' ORDER BY BRID DESC");
    ...
    public function add(){
        ...
        return view('masters.Common.BranchMaster.mstfrm147add',compact(['objCountryList','objIndTypeList','objIndVerList','objGstTypeList','objCurrencyList','objUdfListing','objudfCount','objCompanyList','objBranchGroupList','FormId']));
    }
   ...
   public function save(Request $request){
        ...
        $LOGO               =   NULL;
        $OWNED_FRANCHISE    =   (isset($request['OWNED_FRANCHISE'])!="true" ? 0 : 1);
        $PRICE_LEVEL_REF    =   (isset($request['PRICE_LEVEL_REF']) && trim($request['PRICE_LEVEL_REF']) !="" )? trim($request['PRICE_LEVEL_REF']) : NULL ;
        $FRANCHISEE_LABEL   =   (isset($request['FRANCHISEE_LABEL']) && trim($request['FRANCHISEE_LABEL']) !="" )? trim($request['FRANCHISEE_LABEL']) : NULL ;
        ...
        $IPADDRESS      =   $request->getClientIp();
          ...
          $image_path         =   "docs/company".$CYID_REF."/BranchMaster/Logo";     
        ...
        $array_data   = [
                        ...
                        $USERID,$UPDATE,$UPTIME, $ACTION, $IPADDRESS,$LOGO,$OWNED_FRANCHISE,$PRICE_LEVEL_REF,
                        $FRANCHISEE_LABEL
                    ];
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $LEVELS=isset($user_approval_details[0]->LAVELS) ? $user_approval_details[0]->LAVELS:'';
            
            $user_approval_level = "APPROVAL".$LEVELS;
            //dd($user_approval_level); 
            ...
            $HDR            =   DB::select("SELECT T1.*,T2.PLID,T2.PLCODE,T2.PLNAME FROM TBL_MST_BRANCH T1 LEFT JOIN TBL_MST_PRICE_LEVEL T2 ON T2.PLID=T1.PRICE_LEVEL_REF WHERE T1.BRID='$id'"); 
            $HDR            =   count($HDR) > 0?$HDR[0]:[];

            return view('masters.Common.BranchMaster.mstfrm147edit',compact(['objResponse','user_approval_level','objRights','objCountryList','objIndTypeList','objIndVerList','objGstTypeList','objCurrencyList',
            'objRegCountryName','objRegStateName','objRegCityName','objIndtypeName','objIndVerName','objUDF','objudfCount','objCompanyList','objBranchGroupList','objDataResponse','objCount','FormId','HDR']));
        }
    ...
    public function view($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Common.BranchMaster.mstfrm147view',compact([
                ...
                'objIndVerName','objCompanyList','objBranchGroupList','objDataResponse','objCount','HDR'
                ]));
        }
