...
class MstFrm561Controller extends Controller{

    protected $form_id  =   561;
    protected $vtid_ref =   631;
    protected $view     =   "masters.Sales.PackagePriceList.mstfrm561";
    ...
    public function index(){  
        ...
        LEFT JOIN TBL_MST_PRICE_LEVEL T3 ON T3.PLID=T1.PRICE_LEVEL_REF
        WHERE T1.CYID_REF='$CYID_REF' AND T1.BRID_REF='$BRID_REF' ORDER BY PPL_ID DESC");
       //dd($objDataList);
    ...
    public function save(Request $request){
        ...
        $data2 = array();
        if(isset($request['PACKAGE_REF']) && !empty($request['PACKAGE_REF'])){
            foreach($request['PACKAGE_REF'] as $key=>$val){
                $data2[] = array(
                    'PACKAGE_REF'      => trim($_REQUEST['PACKAGE_REF'][$key])?trim($_REQUEST['PACKAGE_REF'][$key]):NULL,
                    'PRICE'        => trim($_REQUEST['PRICE'][$key])?trim($_REQUEST['PRICE'][$key]):0,
                );
            }
        }
        ...
        $PRICE_LEVEL_REF = $request['PRICE_LEVEL_REF'];
        $DEACTIVATED    =   0;  
        $DODEACTIVATED  =   NULL;  
        $CYID_REF       =   Auth::user()->CYID_REF;
        $BRID_REF       =   Session::get('BRID_REF');
        ...
        $array_data   = [
            $DOC_NO,           $DOC_DT,        $PRICE_LEVEL_REF,    $DEACTIVATED,
            $DODEACTIVATED,    $CYID_REF,      $BRID_REF,           $FYID_REF,  
            $XML,              $VTID_REF,      $USERID,             $UPDATE,    
            $UPTIME,           $ACTION,        $IPADDRESS   
        ];

        //dd($array_data);
        $sp_result = DB::select('EXEC SP_PACKAGE_PRICE_LIST_IN ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?', $array_data);
    ...
    public function edit($id=NULL){
        ...
        if(!is_null($id)){
            ...
            $HDR            =   DB::select("SELECT T1.*,T2.PLID,T2.PLCODE,T2.PLNAME FROM TBL_MST_PACKAGE_PRICE_LIST_HDR T1 LEFT JOIN TBL_MST_PRICE_LEVEL T2 ON T2.PLID=T1.PRICE_LEVEL_REF WHERE T1.PPL_ID='$id'"); 
            //dd($HDR);
            $HDR            =   count($HDR) > 0?$HDR[0]:[];

            $objUdf         =   $this->getUdf(['VTID_REF'=>$this->vtid_ref]);

            $DETAILS        =   DB::select("SELECT T1.*,T2.PKMID,T2.PKMCODE,T2.PKMNAME FROM TBL_MST_PACKAGE_PRICE_LIST_MAT T1 LEFT JOIN TBL_MST_PACKAGE_MASTER T2 ON T2.PKMID=T1.PACKAGE_REF WHERE PPL_ID_REF='$id'"); 
            //dd($DETAILS);
            $objtempUdf     =   $objUdf;
    ...
    public function update(Request $request){
        ...
        $sp_result = DB::select('EXEC SP_PACKAGE_PRICE_LIST_UP ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?', $array_data);
        //dd($sp_result);
        ...
        //     //$sp_result = DB::select('EXEC SP_LATE_DEDUCTION_UP ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?' , $log_data);
        //     $sp_result = DB::select('EXEC SP_PACKAGE_PRICE_LIST_UP ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?', $array_data);
        //     //dd($sp_result);
    ...
    public function cancel(Request $request){
        ...
        $TABLE      =   "TBL_MST_PACKAGE_PRICE_LIST_HDR";
        $FIELD      =   "PPL_ID";
        $ID         =   $id;
        $UPDATE     =   Date('Y-m-d');
        $UPTIME     =   Date('h:i:s.u');
        ...
        $req_data[0]=[
            'NT'  => 'TBL_MST_PACKAGE_PRICE_LIST_HDR',
            'NT'  => 'TBL_MST_PACKAGE_PRICE_LIST_MAT',
        ];
    ...
    public function attachment($id){
        ...
        if(!is_null($id)){
            ...
            $objResponse = DB::table('TBL_MST_PACKAGE_PRICE_LIST_HDR')->where('PPL_ID','=',$id)->first();
    ...
    public function getPriceLevelCheck(Request $request){
        $CYID_REF   =   Auth::user()->CYID_REF;
        $BRID_REF   =   Session::get('BRID_REF');
        $FYID_REF   =   Session::get('FYID_REF');

        $data   =   DB::select("SELECT TOP 1 PRICE_LEVEL_REF AS PRICE_LEVEL  FROM TBL_MST_PACKAGE_PRICE_LIST_HDR 
    ...
    public function getPackage(Request $request){
        ...
        $data   =   DB::select("SELECT * FROM TBL_MST_PACKAGE_MASTER  WHERE  CYID_REF='$CYID_REF' AND STATUS='A' AND (DEACTIVATED=0 OR DEACTIVATED IS NULL)"); 
        //dd($data);
        return Response::json($data);
    }
