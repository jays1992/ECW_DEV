...
class MstFrm145Controller extends Controller
{
    ...
    public function index(){
        ...
        $objDataList    =   DB::select("SELECT T1.DOCNODEFIID,T1.EFFECTIVE_DT,T1.INDATE,T1.DEACTIVATED,T1.DODEACTIVATED,T1.STATUS,T2.VCODE,T2.DESCRIPTIONS,CONCAT(T3.BRCODE,' - ',T3.BRNAME) AS BRANCH_NAME
        FROM TBL_MST_DOCNO_DEFINITION T1
        LEFT JOIN TBL_MST_VOUCHERTYPE T2 ON T1.VTID_REF=T2.VTID
        LEFT JOIN TBL_MST_BRANCH T3 ON T3.BRID=T1.BRID_REF
   ...
   public function save(Request $request){
        ...
        $CYID_REF       =   Auth::user()->CYID_REF;
        //$BRID_REF       =   Session::get('BRID_REF');
        $BRID_REF       =   isset($request['BRANCH_ID'])?$request['BRANCH_ID']:NULL;
        $FYID_REF       =   Session::get('FYID_REF');       
        $VTID           =   $this->vtid_ref;
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $HDR=[];
            if(isset($objResponse->BRID_REF) && $objResponse->BRID_REF !=''){

                $BRID   =   $objResponse->BRID_REF;
                $HDR    =   DB::select("SELECT 
                BRID AS BRANCH_ID,
                CONCAT(BRCODE,' - ',BRNAME) AS BRANCH_NAME
                FROM TBL_MST_BRANCH
                WHERE BRID='$BRID'
                "); 
            }
            ...
            return view('masters.Common.DocumentNumberDefinition.mstfrm145edit',compact(['objResponse','user_approval_level','objRights','objVtList','objVtName','HDR']));
        }
    ...
    public function view($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Common.DocumentNumberDefinition.mstfrm145view',compact(['objResponse','objVtName','HDR']));
        }
