...
class MstFrm198Controller extends Controller
{
    ...
    public function loadsVouchersList(Request $request){
        ...
        $VoucherList =  DB::select('SELECT A.*, C.MODULENAME FROM TBL_MST_VOUCHERTYPE A INNER JOIN TBL_MST_MODULE_VOUCHER_MAP B ON A.VTID = B.VTID_REF
		INNER JOIN TBL_MST_MODULE C ON B.MODULEID_REF = C.MODULEID WHERE  A.VTID IN  (SELECT VTID_REF FROM TBL_MST_ROLEDETAILS)
		AND A.CYID_REF = ?  AND B.MODULEID_REF = ? AND A.STATUS = ? ORDER BY A.VT_SEQUENCE', 
        [$CYID_REF, $module_id,  'A' ]);
       ...
       // DD($CYID_REF.$BRID_REF.$FYID_REF.$id);
        ...
        if(!is_null($id))
        {
            $objSE = DB::table('TBL_MST_ROLE')                          
                            ...
                            $objSEMAT = DB::select("SELECT R.*,V.DESCRIPTIONS,V.VCODE,V.VT_SEQUENCE,M.MODULEID,M.MODULENAME,M.MODULE_SEQUENCE 
								FROM TBL_MST_ROLEDETAILS R INNER JOIN TBL_MST_MODULE_VOUCHER_MAP P ON R.VTID_REF = P.VTID_REF
								INNER JOIN TBL_MST_MODULE M ON P.MODULEID_REF = M.MODULEID INNER JOIN TBL_MST_VOUCHERTYPE V
								ON R.VTID_REF = V.VTID WHERE R.ROLLID_REF = ? ORDER BY M.MODULE_SEQUENCE,V.VT_SEQUENCE",[$id]);    
